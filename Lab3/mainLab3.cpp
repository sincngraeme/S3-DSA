...
int main()
{
	...
	// Checksum a random buffer (non-zeros --> need malloc() since calloc() sets all to zeros and don't need to do this since it takes time and resources - we will replace these)
	...
	for( i=0; i < 1024; ++i ) {
		bPackedString[i] = rand() & 0xff;										// Generate a random single byte number and place in allocated memory
	}
	rc = Checksum( (void *)bPackedString, 1024, CHK_8BIT );
	printf("\n8 bit checksum of buffer is %d.\n", rc); 
	
	// Zap some bits (change it) and report change in checksum  - // get a byte to flip. We'll just zap some middle bit
	rc = rand() % 1024;   														// Choose a number between 0 and 1023 (there are 1024 bytes to choose from that can be altered)
	iBit = rand() % 8;	  														// Choose a number between 0 and 7 (one of the 8 bits is the chosen byte)
	iMask = 1 << iBit;	  														// Left shift 0000 0001 by a random number of bits between 0 and 7 (to select bit)

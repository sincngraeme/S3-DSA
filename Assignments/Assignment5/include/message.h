...
/*
		...
		1. identified struct typedefs from the tree structs.
*/
...
char* GetMessageFromFile(char* buff, int iLen, int randNum, int numQuotes, long int* quoteIndices, int* quoteLengths);  // buff is a character array (need top pass the node element that is a character array)
char* messagemain(int num);